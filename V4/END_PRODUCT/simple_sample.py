...
#    BUY
    if BuyCondition is not None and BuyCondition == True:
        ...
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        print(f' BUY - time: {timestamp}, Close: {price}, MAL: {lma}, MS: {sma} ,SL: {stoploss}, TP: {takeprofit}')
        buy()
        pos = 1
...
#   CONDITION CLOSE BUY   
    if CloseBuy is not None and CloseBuy == True:
        ...
        print(f'SMA CLOSE ({BuyProfit}) - time: {timestamp}, Close: {price}, MAL: {lma}, MS: {sma}, SL:{stoploss}') 
        pos = 0
        lstpos = 1
...
#   OPEN BUY
   # Cprice = df['Close'].iloc[-1]
    if df['SMA'].iloc[-1] > df['LMA'].iloc[-1] and pos == 1:
        ...
        sma = df['SMA'].iloc[-1]  
        pos = 1
        print(f'OPEN BUY - time: {timestamp}, Close: {price}, MAL: {lma}, MS: {sma}, SL: {stoploss}, TP: {takeprofit} ')
